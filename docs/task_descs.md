...
- 硬筛选（当前口径）：
	- 已联系名单命中：硬跳过
	- 输入记录不合法（缺姓名/字段结构错误）：fail fast，显式报错
	- 负向业务信号默认不硬跳过；仅在已抓取文本证据时，命中明确负向关键词（如暂停招生/不招收）才硬跳过
- 软筛选（评分排序）：
	- 信息不足（如无主页、无邮箱、方向描述短）只降分，不硬跳过
- 生成预筛分数与原因标签
- 输出候选优先级列表（Top N 进入任务2）

### 3. 输出
- 预筛结果（包含分数、特征、排序理由、是否跳过）
- 跳过原因（已联系/证据驱动负向信号/超出预算）
- 批量闭环推荐结果（`final_recommendations.json`，包含推荐理由与风险提示）
...
3. 工程化收口：补充“预算上限 + 已联系名单 + skip原因统计”的端到端回归验证。