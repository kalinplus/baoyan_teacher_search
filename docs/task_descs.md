...
- **状态**：已完成
- **说明**：已实现 `teachers + teacher_profiles` 双轨输出，清华 7 个院系 + 上海交大 4 个院系已适配，策略为“自动识别优先，站点规则兜底”。
...
### 任务3：匹配度分析与推荐排序
- **状态**：待实现
- **说明**：当前仅输出 `recommendation_reason` 与 `risk_flags` 占位规则，需继续细化颗粒度与真实样本验证。

### 任务4：CLI / GUI 封装
- **状态**：待实现
- **说明**：CLI 先行已满足；GUI 作为后续增强。

## 当前优先 TODO

1. 推荐规则细化：继续增强 `recommendation_reason` 与 `risk_flags` 规则颗粒度。
2. 结果复核门禁：补充批量闭环后的人工复核清单与抽样机制。
...
- 2026-04-17：清华大学 7 个院系教师页适配完成，SIGS 学科白名单外置到 `config/sigs_subject_keywords.json`。
