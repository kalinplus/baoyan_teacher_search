...
- 当前策略为“学校英文缩写 + 用户输入老师名单次查询 + 第1页 + 首个正则匹配 author_id”，并在首候选后执行两层校验（author_id 反查冲突 + Scholar 主页姓名一致性）。
- 若老师输入包含中文，系统会记录 warning 提示结果可能不准，但不会自动改写输入。

已实现消歧口径：
- 保留首个匹配 author_id 策略，不启用多候选回退。
- 首候选后执行两层校验：
	1) author_id 反查冲突校验：若该 author_id 已在缓存中绑定其他老师姓名，则标记 skip。
	2) Scholar 主页姓名校验：若主页姓名与当前检索老师姓名不一致，则标记 skip。
- 两层都通过时才写入缓存并继续流程。
